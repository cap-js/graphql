const { GraphQLObjectType, GraphQLList, GraphQLInt, GraphQLInterfaceType } = require('graphql')
...
module.exports = cache => {
  const nodeInterfaceFields = Object.freeze({
    // TODO: find solution for: GraphQL types must define one or more fields
  ...
  const getNodeInterface = () => {
    ...
    const cachedNodesInterface = cache.get(name)
    if (cachedNodesInterface) return cachedNodesInterface

    const fields = nodeInterfaceFields
    const newNodesInterface = new GraphQLInterfaceType({ name, fields })
    cache.set(name, newNodesInterface)

    return newNodesInterface
  }

  const connectionInterfaceFields = Object.freeze({
    [CONNECTION_FIELDS.totalCount]: { type: GraphQLInt },
    [CONNECTION_FIELDS.nodes]: { type: new GraphQLList(getNodeInterface()) }
  })

  const getConnectionInterface = () => {
    const name = 'Connection'

    const cachedConnectionInterface = cache.get(name)
    if (cachedConnectionInterface) return cachedConnectionInterface

    const fields = connectionInterfaceFields
    const newConnectionInterface = new GraphQLInterfaceType({ name, fields })
    cache.set(name, newConnectionInterface)

    return newConnectionInterface
  }
  ...
  const entityToObjectConnectionType = entity => {
    ...
    const fields = { ...connectionInterfaceFields }
    const interfaces = [getConnectionInterface()]
    const newEntityObjectConnectionType = new GraphQLObjectType({ name, interfaces, fields: () => fields })
