...
module.exports = cache => {
  ...
  const _serviceToObjectType = service => {
    ...
    for (const key in service.entities) {
      ...
      const entityName = toGQLName(key)
    ...
    return new GraphQLObjectType({ name: toGQLName(service.name) + '_input', fields })
  }
  ...
  const _entityToObjectType = entity => {
    ...
    return new GraphQLObjectType({ name: toGQLName(entity.name) + '_input', fields })
  }
