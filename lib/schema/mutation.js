...
module.exports = cache => {
  ...
  const _entityToObjectType = entity => {
    ...
    const fields = Object.fromEntries(
      Object.entries({
        create: _create(entity, entityObjectType),
        update: _update(entity, entityObjectType, filterInputObjectType),
        delete: _delete(filterInputObjectType)
      }).filter(([k, v]) => v)
    )
  ...
  const _create = (entity, entityObjectType) => {
  ...
  const _update = (entity, entityObjectType, filterInputObjectType) => {
    ...
    if (!updateInputObjectType || !filterInputObjectType) return
  ...
  const _delete = filterInputObjectType => {
    // filterInputObjectType is undefined if the entity only contains elements that are associations or compositions
