...
module.exports = cache => {
  ...
  const _serviceToObjectType = service => {
    ...
    for (const key in operations) {
      ...
      const action = operation.kind === 'action' && operation
      if (!action) continue

      // Ignore non-scalar return types for now
      if (action.returns && !action.returns._type) break

      const gqlScalarType = cdsToGraphQLScalarType(action.returns)
      const type = gqlScalarType

      const args = {}

      for (const key in action.params) {
        const param = action.params[key]
