...
module.exports = cache => {
  ...
  const _update = entity => {
    ...
    if (!entity._isSingleton) args[ARGS.filter] = { type: new GraphQLNonNull(filterInputObjectType) }

    const type = entity._isSingleton ? entityObjectType : new GraphQLList(entityObjectType)
    return { type, args }
  }

  const _delete = entity => {
    if (entity._isSingleton && !entity['@odata.singleton.nullable']) return
