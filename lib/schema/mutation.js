...
const operationGenerator = require('./types/operation')
...
module.exports = cache => {
  ...
  const _serviceToObjectType = service => {
    ...
    for (const key in operations) {
      const action = operations[key].kind === 'action' && operations[key]
      if (!action) continue

      const field = operationGenerator(cache, service).operationToField(action)
      if (!field) continue

      fields[gqlName(key)] = field
    }
