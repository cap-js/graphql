...
const { RELATIONAL_OPERATORS, STRING_OPERATIONS, CONJUNCTION_OPERATIONS_SUPPORT } = require('../../constants')
const {
  GraphQLBinary,
  GraphQLDate,
  GraphQLDateTime,
...
module.exports = cache => {
  ...
  const _generateFilterType = (gqlType, operations) => {
    ...
    const ops = operations.map(op => [
      [op],
      { type: CONJUNCTION_OPERATIONS_SUPPORT[op] ? new GraphQLList(gqlType) : gqlType }
    ])
