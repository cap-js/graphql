const { Kind, visit, typeFromAST, TypeInfo, visitWithTypeInfo } = require('graphql')
...
const _directPath = (node, ancestors) =>
  ancestors
    .concat(node)
    .filter(e => e.kind === Kind.FIELD || e.kind === Kind.OBJECT_FIELD || e.kind === Kind.ARGUMENT)
    .map(a => a.name.value)

// TODO: does not yet work correctly, need to go along fields, args and input objects
const _getTypeFrom_fields = (_fields, names, index = 0) => {
  const name = names[index++]
  const _field = _fields[name]
  const type = _getTypeFrom_fieldOr_arg(_field)

  if (index < names.length) return _getTypeFrom_fields(type.getFields(), names, index)
  ...
  const editedAST = visit(
    ...
    visitWithTypeInfo(typeInfo, {
      ...
      [Kind.OBJECT_FIELD](node, key, parent, path, ancestors) {
        ...
        // const names = _directPath(node, ancestors)
        // const type = _getTypeFrom_fields(_fields, names) // TODO: get type using path and ancestors
