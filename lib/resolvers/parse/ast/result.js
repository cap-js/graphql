...
const formatResult = (field, result, skipTopLevelConnection) => {
    ...
    for (const key in object) {
      ...
      for (const field of fields) {
        ...
        if (Array.isArray(value)) {
          object[responseKey] = _formatArray(field, value)
        } else if (typeof value === 'object' && !Buffer.isBuffer(value)) {
          object[responseKey] = _formatObject(field.selectionSet.selections, value)
        } else {
  ...
  return _formatArray(field, result, skipTopLevelConnection)
}
