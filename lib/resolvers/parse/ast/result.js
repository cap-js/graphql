...
const formatResult = (entity, field, result, isConnection) => {
  const _formatObject = (type, selections, object) => {
    ...
    for (const key in object) {
      ...
      for (const field of fields) {
        ...
        result[responseKey] = _formatByType(element._target || element, field, value, element.is2many)
      }
    }
  ...
  const _formatByType = (type, field, value, skipTopLevelConnection) => {
    if (Array.isArray(value)) return _formatArray(type, field, value, skipTopLevelConnection)
