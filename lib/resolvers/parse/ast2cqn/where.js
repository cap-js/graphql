...
const _objectFieldTo_xpr = (objectField, columnName) => {
  ...
  let where
  if(cdsOperator === GQL_TO_CDS_STRING_OPERATIONS[STRING_OPERATIONS.contains]) where = [_genericFn(cdsOperator, [{ ref: [columnName] },{ val: cdsValue }])]
  else where = [{ ref: [columnName] }, cdsOperator, { val: cdsValue }]
  return where
}

const _genericFn = (methodName, args, operator) => {
