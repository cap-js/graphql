...
const _wrapResolver = (service, resolver, parallel) => async (root, args, context, info) => {
  const response = {}
  ...
  for (const fieldNode of enrichedFieldNodes) {
    for (const field of fieldNode.selectionSet.selections) {
      ...
      response[responseKey] = parallel ? resolver(service, entity, field) : await resolver(service, entity, field)
    }
  }
