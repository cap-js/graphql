...
const { ensureError } = require('./utils')
...
const _wrapResolver = (service, resolver, parallel) =>
  async function CDSRootResolver(root, args, context, info) {
    ...
    const _setResponse = async (response, key, value) => {
      try {
        response[key] = await value
      } catch (e) {
        response[key] = ensureError(e)
      }
    }

    const _getAndSetResponse = async (response, field) => {
      const { key, value } = _getResponse(field)
      await _setResponse(response, key, value)
    }
    ...
    if (parallel) {
      await Promise.all(
        enrichedFieldNodes.map(fieldNode =>
          Promise.allSettled(
            fieldNode.selectionSet.selections.map(async field => await _getAndSetResponse(response, field))
          )
        )
      )
    } else {
      ...
      for (const fieldNode of enrichedFieldNodes) {
        for (const field of fieldNode.selectionSet.selections) {
          await _getAndSetResponse(response, field)
        }
      }
    }
