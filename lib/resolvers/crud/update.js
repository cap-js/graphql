...
module.exports = async (context, service, entity, selection) => {
  ...
  const result = await service.tx(async tx => {
    ...
    resultBeforeUpdate = await tx.dispatch(new cds.Request({ ...{req, res} = context, query: queryBeforeUpdate }))
    if (resultBeforeUpdate.length === 0) return []

    return await tx.dispatch(new cds.Request({ ...{req, res} = context, query }))
  })
