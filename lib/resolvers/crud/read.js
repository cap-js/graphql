...
module.exports = async ({ req, res }, service, entity, selection) => {
  ...
  const is2many = !entity._isSingleton // Return array if entity is not a singleton
  if (!is2many) query.SELECT.one = true

  const isConnection = is2many // Pagination is only possible for more than one result
  const columns = astToColumns(entity, selection.selectionSet.selections, isConnection)
