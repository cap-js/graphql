...
const { IS_PRODUCTION, gqlName } = require('../utils')
...
const _cdsToGraphQLError = (context, err) => {
  ...
  // TODO: which target for multiple errors?
  const target = gqlName(extensions?.target || extensions?.details?.find(d => d.target).target) // extensions?.details?.map(d => d.target)
  return Object.defineProperty(graphQLError, '_cdsError', { value: { target }, writable: false, enumerable: false })
}
...
const formatError = error => {
  const _cdsError = error.originalError?._cdsError
  if (_cdsError) {
    const { target } = _cdsError
    if (target) error.path.push(target)

    return error
  } 

  // CDS errors have already been logged
