...
const queryLogger = (req, _, next) => {
  ...
  // Only add properties to object that aren't undefined or empty objects
  const queryInfo = {}
  if (operationName) queryInfo.operationName = operationName
  if (variables && !_isEmptyObject(variables)) queryInfo.variables = variables
