...
function GraphQLAdapter(options) {
  ...
  const defaults = { graphiql: true, errorFormatterImpl: '@cap-js/graphql/lib/errorFormatter' }
  const schema = generateSchema4(services)
  options = { ...defaults, ...options }

  const errorFormatterImpl = require(options.errorFormatterImpl)
  ...
  router.use((req, res) =>
    createHandler({ schema, context: { req, res, errorFormatterImpl }, formatError, ...options })(req, res)
  )
